...
# Added Signal (?) VSS Actuator ignition switch state (uint8 Enum) - NEW!!
    # Mapping: 0 = LOCK, 1 = OFF, 2 = ACC, 3 = ON
    IGNITION_STATE_SIGNAL =  "Vehicle.LowVoltageSystemState" #"Vehicle.Body.IgnitionState"
    ...
    def run(self):
        ...
        ignition_state = int(self.kuksa.get(self.IGNITION_STATE_SIGNAL, 0))
        ...
        if self.unlock_done:
            ...
            if ignition_state == 2: # Infotainemnt/ Radio activated 
                pass
            
            elif ignition_state == 3 and not self.ignition_on_triggered:
                print("StartSequence Info: Ignition State is ON (3) -> Triggered Check control simulation") 
                self.ignition_on_triggered = True

                # automatically start engine when ignition is active 
                ...
                if not is_engine_running:
                    ...
                    self.kuksa.set(self.ENGINE_RUNNING_SIGNAL, True)

            # safety fallback: reset if user turns key back to LOCK 0 or OFF 1
            elif ignition_state in [0,1]:
                ...
                if bool(self.kuksa.get(self.ENGINE_RUNNING_SIGNAL, False)):
                    self.kuksa.set(self.ENGINE_RUNNING_SIGNAL, False)
